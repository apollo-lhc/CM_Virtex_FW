...
architecture behavioral of VIRTEX_TCDS_interface is
  ...
  signal reg_data :  slv32_array_t(integer range 0 to 80);
  constant Default_reg_data : slv32_array_t(integer range 0 to 80) := (others => x"00000000");
  ...
  begin  -- process reads
    ...
    if localRdReq = '1' then
      ...
      case to_integer(unsigned(localAddress(6 downto 0))) is
        ...
        when 80 => --0x50
          localRdData( 7 downto  0)  <=  Mon.PRMEMC.ERROR;                   --
          localRdData(15 downto  8)  <=  Mon.PRMEMC.PASS;                    --
        when 71 => --0x47
